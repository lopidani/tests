...
def fme(nr,p=1):
    ...
    for i in range(len(str(nr))):
        ...
        if i == len(str(nr))-1:
           # if product of multiplication len is more than two digits (we must use ciclic the same function fme)
           if len(str(p)) >=2:
              # than we must reuse the function
              s=p 
           # if p has 2 digits we must check the result of multiplication of those digits   
           if len(str(p))==2:
              # if the result of multipication of those digits has 2 digits or more we must continue with multiply
